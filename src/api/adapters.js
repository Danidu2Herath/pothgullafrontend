...
const PALETTE = ['#15803d', '#7c3aed', '#0e7490', '#b45309', '#9f1239', '#1d4ed8', '#374151', '#db2777', '#0f766e'];
...
export function adaptBook(b) {
  const available = b._count?.copies ?? b.copies?.filter?.((c) => c.status === 'AVAILABLE').length ?? 0;
...
export function adaptDevice(d) {
  return {
    ...
    blurb: `Tier ${d.deviceTier} device. ${d.deviceTier >= 4 ? 'Requires staff approval to check out.' : ''}`.trim(),
    tier: d.deviceTier,
    status: d.status,
    raw: d,
  };
...
export function adaptBooking(b) {
  ...
  return {
    ...
    resourceId,
    title: b.bookTitle?.title ?? b.device?.name ?? b.studyRoom?.name ?? b.resourceType,
    userId: b.userId,
...
export function adaptWaitlistEntry(w) {
  return {
    ...
    status: w.status,
    title: w.booking?.bookTitle?.title ?? w.booking?.device?.name ?? w.booking?.studyRoom?.name ?? w.resourceType,
