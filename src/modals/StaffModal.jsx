...
import { createBook, addCopy, createDevice, createRoom } from '../api/catalogue';

const TYPES = ['Book', 'Device', 'Study Room'];
const CATS_BOOK = ['Programming', 'Systems', 'Science', 'History', 'Literature', 'Other'];
...
export default function StaffModal() {
  ...
  function slug(s) {
    return (s || 'item').toUpperCase().replace(/[^A-Z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 10);
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (type === 'Book') {
        const created = await createBook({ title: title.trim(), author: author.trim() || 'Unknown', description: author.trim(), tags: cat ? [cat] : [] });
        const base = slug(title);
        for (let i = 1; i <= n; i++) {
          await addCopy(created.id, `${base}-${Date.now().toString().slice(-5)}-${i}`);
        }
      } else if (type === 'Device') {
        await createDevice({ name: title.trim(), assetTag: `DEV-${slug(title)}-${Date.now().toString().slice(-5)}`, deviceTier: devTier });
      } else {
        await createRoom({ name: title.trim(), capacity: n, features: cat ? [cat] : [], roomQr: `ROOM-${slug(title)}-${Date.now().toString().slice(-5)}` });
      }
  ...
  const catOptions = type === 'Device' ? CATS_DEVICE : type === 'Study Room' ? CATS_ROOM : CATS_BOOK;
  const authorLabel = type === 'Book' ? 'Author' : type === 'Device' ? 'Description / Model' : 'Description';
  const copiesLabel = type === 'Study Room' ? 'Quantity / rooms' : 'Number of copies';
    ...
    <div
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 24px 16px', borderBottom: '1px solid #f0f0f6' }}>
          <h2 style={{ fontFamily: "'Spectral', serif", fontSize: 18, fontWeight: 700, color: '#1a1b2e', margin: 0 }}>Add resource</h2>
          <button onClick={close} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, borderRadius: 6 }}>
        ...
        <div style={{ padding: '20px 24px' }}>
          ...
          <Field label="Title">
            <input
              type="text"
              placeholder={type === 'Book' ? 'Book title' : type === 'Device' ? 'Device name' : 'Room name'}
              value={title}
          ...
          <Field label={authorLabel}>
            <input
              type="text"
              placeholder={type === 'Book' ? 'Author name' : type === 'Device' ? 'e.g. Apple · M3 · 16GB' : 'e.g. Capacity: 8 · Whiteboard + Screen'}
              value={author}
